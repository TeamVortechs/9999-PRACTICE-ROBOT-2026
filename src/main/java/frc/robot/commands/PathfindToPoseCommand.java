...
public class PathfindToPoseCommand extends Command {
  ...
  // PIDController
  private final PIDController translationController = new PIDController(6, 0, 0);

  private final PIDController thetaController = new PIDController(40, 0, 0);

  private final double translationTolerance = 0.05;
  private final double rotationTolerance = 0.05;
  ...
  private ChassisSpeeds calculateFieldRelativeSpeedsToTarget(boolean isFlipped) {
    ...
    xVelocity = MathUtil.clamp(xVelocity, -2, 2);

    yVelocity = MathUtil.clamp(yVelocity, -2, 2);
